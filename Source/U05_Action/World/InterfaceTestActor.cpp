...
void AInterfaceTestActor::BeginFocus()
{
	if (Mesh)
	{
		/*render depth ? buffer or lendering layers ? ,creating highlight event*/
...
{
	UE_LOG(LogTemp, Warning, TEXT("Calling BeginInteract override on interfeace test actor."));
}

void AInterfaceTestActor::EndInteract()
{
	UE_LOG(LogTemp, Warning, TEXT("Calling EndInteract override on interfeace test actor."));
